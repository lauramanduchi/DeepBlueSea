...
class BaselineTrainer(BaseTrain):
    ...
    def train_step(self):
        feed_dict = {self.model.handle: self.sess.run(self.data.iterator.string_handle())}
        _, loss, summaries = self.sess.run([self.model.train_step, self.model.cross_entropy, self.model.summaries],
                                           feed_dict=feed_dict)
