...
class DataGenerator:
    ...
    def get_y_data(self, filename):
        ...
        for box_idx in range(n_boxes):
            ...
            y_reg[:, :, 4 * box_idx + 0] = round((box[2] + box[0]) / 2)  # x-centre
            y_reg[:, :, 4 * box_idx + 1] = round((box[3] + box[1]) / 2)  # y-centre
            y_reg[:, :, 4 * box_idx + 2] = box[2] - box[0]  # width
            y_reg[:, :, 4 * box_idx + 3] = box[3] - box[1]  # height
