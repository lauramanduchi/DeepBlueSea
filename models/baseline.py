...
class Baseline(BaseModel):
    ...
    def build_model(self):
        ...
        layer_conv1 = self.create_convolutional_layer(input=self.x,
                                                 ...
                                                 conv_filter_size=config.filter_size_conv1,
                                                 num_filters=config.num_filters_conv1)

        layer_conv2 = create_convolutional_layer(input=layer_conv1,
                                                 num_input_channels=config.num_filters_conv1,
                                                 conv_filter_size=config.filter_size_conv2,
                                                 num_filters=config.num_filters_conv2)

        layer_conv3 = create_convolutional_layer(input=layer_conv2,
                                                 num_input_channels=config.num_filters_conv2,
                                                 conv_filter_size=config.filter_size_conv3,
                                                 num_filters=config.num_filters_conv3)

        layer_flat = create_flatten_layer(layer_conv3)

        layer_fc1 = create_fc_layer(input=layer_flat,
                                    num_inputs=layer_flat.get_shape()[1:4].num_elements(),
                                    num_outputs=config.fc_layer_size,
                                    use_relu=True)

        layer_fc2 = create_fc_layer(input=layer_fc1,
                                    num_inputs=config.fc_layer_size,
                                    num_outputs=2,
                                    use_relu=False)
